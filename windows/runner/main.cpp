...
int APIENTRY wWinMain(_In_ HINSTANCE instance, _In_opt_ HINSTANCE prev,
  ...
  // Attach to console when present or create when running with debugger
  ...
  if (!window.Create(L"FlClash-Mod", origin, size)) {
    return EXIT_FAILURE;
  }
...
int APIENTRY WinMain(HINSTANCE instance, HINSTANCE prev, LPSTR command_line, int show_command) {
  return wWinMain(instance, prev, GetCommandLineW(), show_command);
}
